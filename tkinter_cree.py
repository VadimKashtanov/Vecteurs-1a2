...
class DraggableFrame(tk.Frame):
	...
	def mettre_a_jour_module(self):
		for x in range(len(self.xs)):
			self.module.X[x] = int(self.xs[x].val.get())
			#print(self.module.__class__, self.module.X)
		for p in range(len(self.ps)):
			self.module.params[self.ps[p].A] = int(self.ps[p].val.get())
		for y in range(len(self.ys)):
			self.module.Y[y] = int(self.ys[y].val.get())
...
class DraggableApp(tk.Tk):
	...
	def mise_a_jour_sortie(self):
		self.vraie_sortie = int(self.la_sortie.val.get())
	...
	def modules_vers_mdl(self):
		copier_dico = lambda di: {k:v for k,v in di.items()}
		#
		...
		for f in self.frames:
			...
			for _l in m.ix:
				...
				ix += [l]
				#
				l['x'] = [(depart+m.ix.index(x) if x!=None else None) for x in l['x']]
			#
			depart_i += [depart]
			depart += len(m.ix)
